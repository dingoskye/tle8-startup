...
const CreateGroup = () => {
    ...
    const {fetchUsers, users} = useLogin()
    const [selectedMembers, setSelectedMembers] = useState(null);
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const [errors, setErrors] = useState({});
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [submitMessage, setSubmitMessage] = useState(null);
    const {apiFetch, loginData, token} = useApi();
    ...
    useEffect(() => {
        if (loginData?.id && users && users.length > 0) {
            const filteredUsers = users.filter(
                user => String(user.id) !== String(loginData.id)
            );
            setAvailableUsers(filteredUsers);
            setSelectedMembers([loginData])
        }
    }, [users, loginData])
    ...
    const handleCreateGroup = async (e) => {
        ...
        try {
            ...
            formData.append('members', JSON.stringify(newSelectedMembers.map(m => m.id)));
        ...
        <>
            ...
            <form className="flex flex-col items-center gap-6 w-full" onSubmit={handleCreateGroup}>
                ...
                <Card variant="tertiary">
                    <label htmlFor="name" className="font-headers text-xl">Naam:</label>
                    {errors.name &&
                ...
                <Card variant="quaternary">
                    <label htmlFor="description" className="font-headers text-xl">Beschrijving:</label>
                    ...
                    <div className="relative">
                        ...
                        <textarea
                            ...
                            name="description"
                            placeholder="Geef hier een korte beschrijving over jouw groep"
                ...
                <Card variant="secondary">
                    <label htmlFor="photo" className="font-headers text-xl">Foto:</label>
                ...
                <Card variant="white">
                    <label className="text-xl font-headers">Leden:</label>
                    ...
                    <div
                        ...
                        {selectedMembers && selectedMembers.length > 0 ? selectedMembers.map((member) => (
                            <div key={member.id}
                                ...
                                {/* Placeholder Avatar */}
                                <User
                                    className="w-10 h-10 mb-1 mt-4 text-text  bg-bg-white border-2 border-text rounded-full p-1"/>
                        ...
                        <div className="relative">
                            ...
                            {isMenuOpen && (
                                <>
                                    ...
                                    <div
                                        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 sm:absolute sm:top-14 sm:left-0 sm:translate-x-0 sm:translate-y-0 bg-bg-white shadow-2xl rounded-md w-[85vw] max-w-sm sm:w-56 z-50 border border-gray-200 overflow-hidden flex flex-col">

                                        {/* SEARCH INPUT */}
                                        <div className="p-2 border-b bg-bg-white">
                                            <input
                                                type="text"
